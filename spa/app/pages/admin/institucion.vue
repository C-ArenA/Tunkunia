...
import { updateInstitutionMutation } from "#shared/clientV1/@pinia/colada.gen";
import type { Institution } from "#shared/clientV1/types.gen";
import { defaultInstitution, useInstitution } from "~/composables/useInstitution";

const { data, status, error: loadError, refresh } = useInstitution();
const form = ref<Institution>({ ...defaultInstitution });
const saved = ref(false);
const error = ref("");
const mutation = useMutation(updateInstitutionMutation());
...
async function save() {
  ...
  saved.value = false;
  try {
    await mutation.mutateAsync({ body: form.value });
    await refresh();
    saved.value = true;
    setTimeout(() => (saved.value = false), 2500);
  } catch {
    error.value = "No se pudo actualizar la información institucional.";
  }
}
</script>
...
<template>
  <UDashboardPanel>
    ...
    <template #body>
      ...
      <UAlert v-else-if="loadError" color="error" title="No pudimos cargar la institución" />
      <template v-else>
        <UAlert
          v-if="saved"
          color="success"
        ...
        <div class="mt-8 grid gap-6 xl:grid-cols-[1fr_320px]">
          <div class="surface grid gap-5 p-6 md:grid-cols-2">
            <UFormField label="Nombre" required class="md:col-span-2"
              ><UInput v-model="form.name" class="w-full"
            /></UFormField>
            <UFormField label="Sigla" required
              ><UInput v-model="form.acronym" class="w-full"
            /></UFormField>
            <UFormField label="URL del logotipo"
              ><UInput v-model="form.logoUrl" class="w-full"
            /></UFormField>
          ...
          <aside class="surface h-fit p-6">
            ...
            >
              ...
              <p v-if="form.description" class="mt-2 text-sm text-white/75">
                {{ form.description }}
              </p>
