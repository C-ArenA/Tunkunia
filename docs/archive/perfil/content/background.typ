...
A continuación se hace un repaso histórico en torno al trámite, su relevancia, sus problemas originales, la necesidad de adopción de tecnologías de la información por el gobierno boliviano, la atención a dicha necesidad en el sistema #acr(
  "SIAI",
) mediante la consultora 2IES y cómo esto deriva posteriormente en la identificación de características comunes a diversos trámites (@fig:background).
...
que según el sociólogo alemán Max Weber, no sólo es la forma más avanzada de ejercer o empuñar el poder por aquellos que lo controlan @watersWeberRationalismModern2015[pág.
  114], sino que también es una *forma racional de organización*, que en su opinión es la forma más pura de sistema legal de autoridad, es necesaria y algunas de sus características fundamentales son las jerarquías, la especialización y la definición estricta de *reglas y regulaciones* @archerDictionaryPublicAdministration2022.
...
Su caso fue motivo de preocupación y cuando finalmente logró recibir su cédula (el cual era el motivo del trámite), no le quedaron más que dos semanas antes de fallecer @charoskyQuejaComoEnergia2014[pag.
  23].
...
el *gobierno electrónico #footnote(
  governmentFootNote,
...
Los esfuerzos por digitalizar los trámites se enmarcarían dentro de este concepto.

Sin embargo, la implementación de un gobierno electrónico no es una tarea sencilla, ya que requiere la realización de esfuerzos en conjunto dentro de los gobiernos.
En Bolivia, a partir del año 2009, se han promulgado decretos, leyes y reglamentos que buscan promover el uso de las tecnologías de la información y comunicación en la administración pública, pero aún queda un largo camino por recorrer para lograr una implementación efectiva y eficiente.
...
Adicionalmente, dada la situación de emergencia sanitaria provocada por la pandemia de COVID-19, el gobierno boliviano ha acelerado la implementación de #acr(
  "TICs",
) en la administración pública, promoviendo el uso de plataformas digitales para la atención al ciudadano y la gestión de trámites.
Por esto mismo, se aprobó, mediante Resolución Ministerial Nº 079/20, el documento de "Lineamientos y estándares técnicos para el desarrollo e implementación de Servicios Digitales", mismo que no sólo insiste en la digitalización de servicios como el trámite, sino que proporciona una guía a las entidades públicas y privadas que desarrollan software bajo este propósito, indicando la disponibilidad de diferentes herramientas creadas por la AGETIC, como la de Ciudadanía Digital.
...
Como puede notarse, aunque se lista solamente lo considerado más relevante, existe bastante normativa y documentación que promueve y/o demanda el uso de las #acr(
  "TICs",
) y la digitalización de trámites en el gobierno boliviano.
...
La ejecución del proyecto, licitado por el #acr(
  "MDPyEP",
), estuvo entonces a cargo de la consultora 2IES,
que procedió a la implementación del nuevo #acr(
  "SIAI",
) atendiendo a las especificaciones técnicas del Documento Base de Contratación,
así como a las distintas interacciones efectuadas con el cliente.

El sistema fue construido como una aplicación web con la arquitectura cliente-servidor, con un backend monolítico y un frontend elaborado como una #acr(
  "SPA",
).
En base a esto y para poder responder a necesidades de integración e interoperabilidad futuras, se optó además por una comunicación entre el frontend y el backend mediante una #acr(
  "API",
) de tipo #acr("REST"), abierta y bien documentada.
...
La capa de negocio, sin embargo, interopera a su vez con otros sistemas mediante servicios #acr(
  "REST",
) #acr("API")
...
El backend fue desarrollado utilizando el lenguaje de programación PHP, con uno de los frameworks de desarrollo más populares y completos de su ecosistema, Laravel.
El frontend, por su lado, fue desarrollado utilizando el framework Vue.js, que permite crear aplicaciones web interactivas y dinámicas.
Además, se utilizó el sistema de gestión de bases de datos PostgreSQL, de tipo relacional.

Inicialmente, se debe mencionar que el sistema #acr(
  "SIAI",
) atiende una amplia variedad de requerimientos, de los cuales son relevantes en este documento aquellos de tipo administrativo, que involucran un grupo de documentos ambientales a ser obtenidos y actualizados por las industrias manufactureras del país, dependiendo de ciertas características y condiciones dadas a conocer en un primer trámite de registro (RAI).

Es decir, el SIAI tiene como protagonistas a las industrias manufactureras que deben cumplir con la normativa ambiental vigente de acuerdo al #acr(
  "RASIM",
).
Esta normativa demanda la obtención de una serie de documentos llamados #acr(
  "IRAP",
  plural: true,
...
Además de atender a los requerimientos iniciales del DBC, 2IES propuso funcionalidades que modernicen el trabajo con el SIAI.
A partir del requerimiento de hacer seguimiento a las solicitudes de distintos documentos ambientales y la generación de los mismos, se identificó a estos procesos como trámites, los cuales debían ser digitalizados.
