...
presuponer una solución técnica.
Al compararlas desde la perspectiva del
...
Delimita los conceptos que Tunkunia necesita
para definir y ejecutar trámites distintos mediante configuración.
Las reglas
...
estado, pero no imponen una técnica de modelado.
Los diagramas de flujo y de
...
de una entidad.
Estas alternativas resultan útiles para documentación, pero no
...
de cada ejecución.
Su aplicación a sistemas de gestión de flujos de trabajo se
encuentra desarrollada por Van der Aalst
@vanderaalstAPPLICATIONPETRINETS1998.
La elección se realiza en diseño y no
...
varias versiones de su proceso.
Cada versión comprende cuatro elementos: una red
...
o transición.
Mientras permanece como borrador puede editarse y validarse; al
publicarse se vuelve inmutable.
El archivado impide iniciar casos nuevos, pero no
...
@vanderaalstAPPLICATIONPETRINETS1998.
Los lugares representan situaciones del
procedimiento y las transiciones, acciones que permiten avanzar.
Un caso posee
...
expediente.
Así, Petrunia se limita a decidir si una transición está habilitada y
...
usuario.
Al iniciar un caso, la persona iniciadora ocupa el recurso ciudadano y
...
envío.
En el prototipo el campo es textual; su metadato permitirá incorporar
posteriormente variantes numéricas, documentales u otras sin alterar la
semántica del disparo.
Cuando la transición representa una consulta externa, el
cliente simulado únicamente precarga ese campo.
El participante debe revisar el
...
autorizado.
Un administrador puede omitir la última condición únicamente como
acción de emergencia, proporcionando una justificación.
El mecanismo no permite
...
de la misma red.
Al vencer, un trabajador interno elimina todos los tokens del
color del caso y coloca uno en el sumidero configurado.
Esta operación es
idempotente, afecta únicamente al caso correspondiente y genera una actuación de
auditoría.
El trabajador busca vencimientos al arrancar y después de forma
...
actuaciones con la entrada, salida, actor y fecha.
La primera facilita la consulta
y la segunda permite explicar cómo se alcanzó ese estado.
Auditoría añade una
...
las rutas proporcionadas por el usuario nunca se utilizan directamente.
La
...
caso o ejercer una función administrativa expresamente autorizada.
La excepción
de emergencia no se confunde con el acceso ordinario y siempre queda justificada
y auditada.

La API HTTP se describe mediante OpenAPI 3.1 @openapi310.
Las operaciones de la
aplicación utilizan la sesión y las reglas anteriores.
...
ficticia.
La clave no concede acceso a expedientes ni sustituye la autorización
de usuarios.

Las otras integraciones también respetan el alcance académico.
Tunkunia produce
un reporte descargable con los datos necesarios para que la institución actualice
su catálogo en gob.bo, sin afirmar una sincronización directa.
La acción de
consulta externa depende de una interfaz de cliente, pero el adaptador del
prototipo devuelve un dato simulado que precarga el campo de la transición.
Una
...
Notificaciones conserva una bandeja interna.
Se genera un aviso cuando una tarea
queda disponible para un participante y, para el ciudadano, cuando cambia el
estado de su caso.
El aviso es información derivada de una actuación confirmada;
no sustituye al historial ni constituye evidencia de que la persona lo haya
leído.
La entrega por correo u otros canales queda fuera del prototipo.

=== Interfaz y límites del prototipo

La SPA organiza la experiencia en tres áreas.
El área pública presenta el
catálogo y el detalle de cada trámite.
El área del participante reúne los casos,
...
existencia de los metadatos requeridos.
La interfaz de ejecución no expone la
...
campo, navegación por teclado y estados visibles de carga, éxito y error.
Estos
criterios son especialmente importantes para ciudadanos que utilizan el sistema
de manera ocasional.
No se declara conformidad completa con un estándar de
...
enchufables.
Tampoco implementa colores que transporten datos ni transiciones
automáticas generales: el color identifica al caso y solo los vencimientos se
procesan sin intervención humana.
Estas restricciones mantienen verificable el
núcleo elegido.

La evolución puede incorporar nuevos tipos de campo, adaptadores externos reales
y canales de notificación sin modificar la semántica de los casos.
Petrunia
podría adquirir versionado independiente mientras conserve su contrato puro de
red y marcado.
Estas posibilidades son direcciones de evolución, no compromisos
...
un trámite genérico.
La red permite reconocer el estado y las acciones del caso;
al seleccionar sus elementos se abren ventanas con la información del expediente
o con el formulario necesario para ejecutar una transición.
Cada acción muestra
además el participante autorizado, de modo que la disponibilidad de una
transición no se confunda con el permiso para dispararla.
Esta vista complementa
