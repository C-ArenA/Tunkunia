...
correspondencia entre sus elementos ejecutables.
La exposición se concentra en
...
La construcción se realizó de forma incremental.
Primero se establecieron el
...
la edición de procedimientos y las vistas de seguimiento.
...
de participantes y de administración.
Los servicios locales de identidad y
...
abiertos y una separación clara entre dominio, transporte y persistencia.
La
...
limita la composición a los puntos de entrada.
Los módulos intercambian tipos o
...
migraciones y conecta los servicios con sus adaptadores HTTP.
Sobre el enrutador
...
A partir del contrato se
generaron los tipos y las interfaces estrictas del servidor.
Un _middleware_
valida cada solicitud contra la especificación antes de ejecutar el manejador y
convierte los fallos en respuestas HTTP uniformes.
Los manejadores sólo realizan
...
El inicio de sesión emplea el flujo de código de autorización de OIDC.
El
...
`email_verified` y `name`.
Después emite un JWT firmado que se conserva en una
_cookie_ segura, `HttpOnly` y `SameSite=Lax`.

Cada solicitud recupera la sesión desde la _cookie_ o desde el encabezado de
autorización.
La identidad resultante se incorpora al contexto HTTP y la
validación OpenAPI rechaza las operaciones protegidas cuando no existe un
principal autenticado.
En el cliente, los _middleware_ de Nuxt protegen las
rutas de participante y verifican el rol `admin` para las rutas de gestión.

=== Implementación de la aplicación web

La SPA se construyó con Nuxt en modo cliente.
Tres _layouts_ separan el portal
público, el espacio de participantes y la administración.
Las reglas de ruta
...
disponibles y presenta su descripción y procedimiento.
El área autenticada
permite buscar trámites, iniciar un caso, consultar su avance e historial y
atender tareas personales.
El área administrativa reúne el catálogo, la edición
...
Las consultas declarativas de Pinia Colada mantienen el
...
eliminación.
De este modo, el cliente y el servidor comparten las mismas formas
...
protocolo previsto para un servicio externo de identidad.
Su configuración
registra a Tunkunia como cliente, habilita usuarios locales de prueba y expone
los puntos de autorización y emisión de tokens.
Esta sustitución permite probar
...
servidor Go.
Esto permite usar _cookies_ seguras durante el desarrollo y evita
diferencias innecesarias con la terminación TLS de un despliegue.
La base de
...
archivos `.env`.
Los valores admitidos y sus predeterminados se declaran en
...
de la documentación.
El resultado desplegable comprende el ejecutable Go, los
...
representación interactiva del procedimiento.
La separación modular y el uso de
...
consulta, inicio y seguimiento de trámites.
Los datos demostrativos permiten
recorrer estas funciones sin utilizar información institucional real.
La
