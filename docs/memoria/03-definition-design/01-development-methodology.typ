...
- *Artefactos evolutivos*: En RUP se deben presentar artefactos al finalizar cada fase. Aquí se propone la presentación de una primera versión o boceto de cada artefacto durante la fase correspondiente, pero estos podrán seguirse elaborando hasta el final del proyecto, para ser introducidos en esta memoria en su versión final.
- *Viajar Liviano*: Se hace un ajuste a los artefactos esperados del proyecto. Se reducen muchos y se añaden otros.
...
En Tunkunia sólo existe un único desarrollador y gestor del proyecto que, además, haría las veces de cliente desde un marco SCRUM. Esto sumado al objetivo de presentar este proyecto como uno de software libre, hace que laa figura del dictador benevolente sea relevante.
...
- *Tutor*: Es un docente de la carrera que se involucra en el desarrollo del proyecto de grado y que a su vez lo avala, convirtiéndose en un coautor del mismo, por lo que está ampliamente interesado en su correcta ejecución. Esto implica que *supervisará* de cerca su desarrollo, lo guiará, intervendrá de ser necesario y seguirá lo señalado en el reglamento de proyecto de grado de ETN 1040.
...
#figure(
  table(
    ...
    inset: (.3em, .6em),
    table.header("Disciplina", "Artefacto", "Inicio", "Elaboración", "Construcción", "Transición"),
    table.cell(rowspan: 2)[Gestión del\ Proyecto],
    [Metodología], table.cell(colspan: 3, fill: rgb(200, 250, 150))[], [],
    [Cronograma del Proyecto], table.cell(colspan: 4, fill: rgb(200, 250, 150))[],
    table.cell(rowspan: 3)[Modelado del\ Negocio],
    [Perfil de Proyecto], table.cell(fill: rgb(200, 250, 150))[], [], [], [],
    [Lista de Trámites Modelados], [], table.cell(colspan: 3, fill: rgb(200, 250, 150))[],
    [Lista de Procedimientos], [], table.cell(colspan: 3, fill: rgb(200, 250, 150))[],
    table.cell(rowspan: 3)[Requerimientos],
    [Backlog de Historias de Usuario], table.cell(colspan: 4, fill: rgb(200, 250, 150))[],
    [Modelo de Casos de Uso], [], table.cell(colspan: 3, fill: rgb(200, 250, 150))[],
    ...
    [Selección de Tecnologías], [], table.cell(colspan: 3, fill: rgb(200, 250, 150))[],
  ),
