...
func TestCitizenAndServantExecuteWorkflow(t *testing.T) {
	...
	db := testDB(t)
	catalogRepo := catalog.NewSQLiteRepository(db, sqlc.New())
	procedureService := catalog.NewProcedureService(catalogRepo)
	tramiteID := insertFixture(t, db)
	_, err := catalogRepo.SaveDraftProcedure(ctx, tramiteID, citizenServantNet())
	require.NoError(t, err)
	_, err = procedureService.PublishProcedure(ctx, tramiteID)
	require.NoError(t, err)
	repo := NewSQLiteRepository(db, sqlc.New())
	service := NewService(repo, catalogRepo)
	...
	unassigned, err := repo.ListUnassigned(ctx)
	require.NoError(t, err)
	...
	notifications, err := repo.ListNotifications(ctx, 2)
	require.NoError(t, err)
...
func TestFireRejectsWrongActorAndStaleRevision(t *testing.T) {
	...
	_, err = procedureService.PublishProcedure(ctx, tramiteID)
	require.NoError(t, err)
	service := NewService(NewSQLiteRepository(db, sqlc.New()), catalogRepo)
	item, err := service.Start(ctx, tramiteID, 1)
	require.NoError(t, err)
